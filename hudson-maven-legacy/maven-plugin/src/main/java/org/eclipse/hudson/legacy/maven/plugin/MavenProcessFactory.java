...
final class MavenProcessFactory extends AbstractMavenProcessFactory implements ProcessCache.Factory {
    ...
    protected ArgumentListBuilder buildMavenAgentCmdLine(BuildListener listener,int tcpPort) throws IOException, InterruptedException {
        ...
        // interceptor.jar
        args.add(isMaster?
            Which.jarFile(org.eclipse.hudson.legacy.maven.interceptor.AbortException.class).getAbsolutePath():
    ...
    public static boolean profile = Boolean.getBoolean("org.eclipse.hudson.legacy.maven.plugin.profile");
    
    /**
     * If true, launch Maven with YJP offline profiler agent.
     */
    public static boolean yjp = Boolean.getBoolean("org.eclipse.hudson.legacy.maven.plugin.yjp");

    static {
        String port = System.getProperty("org.eclipse.hudson.legacy.maven.plugin.debugPort");
        if(port!=null)
            debugPort = Integer.parseInt(port);
    }
    
    public static int socketTimeOut = Integer.parseInt( System.getProperty( "org.eclipse.hudson.legacy.maven.plugin.socketTimeOut", Integer.toString( 30*1000 ) ) );
