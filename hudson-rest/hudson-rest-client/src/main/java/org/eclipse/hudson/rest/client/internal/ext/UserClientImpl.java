...
/**
 * {@link org.eclipse.hudson.rest.client.ext.UserClient} implementation.
 *
    ...
    implements UserClient
{
    protected UriBuilder uri() {
        return getClient().uri().path("users");
    }

    public UserDTO getUser(final String userName) {
        ClientResponse resp = resource(uri().path(userName)).get(ClientResponse.class);
        try {
            ensureStatus(resp, OK);
            return resp.getEntity(UserDTO.class);
        }
        finally {
            close(resp);
        }
